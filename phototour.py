...
#                 patches = norm_patches(patches)
            ...
            elif self.mode == 'Quadruwithglobal':
                idx_a = random.randrange(self.data_len)
                a_label = self.labels[idx_a]
                idx_n1 = random.randrange(self.data_len)
                n1_label = self.labels[idx_n1]
                idx_n2=random.randrange(self.data_len)
                n2_label = self.labels[idx_n2]

                while n1_label == a_label or n2_label == a_label:
                    while n1_label == n2_label :
                        idx_n2 = random.randrange(self.data_len)
                        n2_label = self.labels[idx_n2]
                    idx_n1=random.randrange(self.data_len)
                    n1_label=self.labels[idx_n1]
                ...
                data_a, data_p, data_n1, data_n2 = self.data[idx_a], self.data[idx_p], self.data[idx_n1],self.data[idx_n2]
                patches = [data_a, data_p, data_n1,data_n2]
                patches = [p.numpy() for p in patches]
                if self.augment:
                    patches = augment_patches(patches)
                patches = resize_patches(patches, 32)
